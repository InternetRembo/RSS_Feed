...
const Articles: React.FC<ArticlesProps> = ({setIsUserAuth}: ArticlesProps) => {
	...
	return (
		<div className={'flex justify-center'}>
			...
			{isModalOpen && <ClickOutsideHandler onAwayClick={() => {
                ...
                <TopicModal initialData={topicToUpdate} onTopicUpdate={onTopicUpdate} onTopicCreate={onTopicCreate}
                            mode={mode} onClose={() => {
					setIsModalOpen(false)
				}}/>
            </ClickOutsideHandler>}
			...
			<header className={'bg-white w-full h-[60px] fixed top-o left-0 border-b z-50'}>
				...
				<button
					className={'w-[150px] h-[40px] bg-purple-500 hover:bg-purple-600 text-white rounded-lg absolute top-3 right-5'}
					onClick={() => {
			...
			<div className={'fixed top-16 left-8 gap-3 flex flex-col '}>
				<div className={'text-white bg-purple-500 p-6 shadow-2xl w-[360px] h-[420px] rounded-md text-center'}>
					...
					<button onClick={(e) => {
						...
						setIsModalOpen(true);
					}} className={'mt-4 bg-white text-purple-500 p-1 rounded-md w-[140px] text-lg hover:underline'}>Add
						topic
					</button>
			...
			{loading ? (
				...
				<div className={'mt-16'}>
					{articles && articles.length > 0 ? (
						sortedArticles.map((el: article, index) => (
							<Article
								link={el.link}
								key={el.title + index}
								title={el.title}
						...
						<span className="text-4xl font-medium">Please select topics</span>
